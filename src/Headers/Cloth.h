...
class Cloth {
  public:
    ...
    enum DrawModeEnum { DRAW_NODES, DRAW_LINES, DRAW_FACES };
  ...
  public:
    ...
    void pin(Vec2 index, Vec3 offset)  // Pin cloth's (x, y) node with offset
    {
        if (!(index.x < 0 || index.x >= nodesPerRow || index.y < 0 || index.y >= nodesPerCol)) {
            ...
            getNode(index.x, index.y)->mass = std::numeric_limits<double>::infinity();
            getNode(index.x, index.y)->w = 0.0;
        }
    }
    void unPin(Vec2 index)  // Unpin cloth's (x, y) node
    {
        if (!(index.x < 0 || index.x >= nodesPerRow || index.y < 0 || index.y >= nodesPerCol)) {
            ...
            getNode(index.x, index.y)->mass = 1.0;
            getNode(index.x, index.y)->w = 1.0;
        }
    }
    ...
    void init() {
        ...
        for (int i = 0; i < nodesPerRow; i++) {
            for (int j = 0; j < nodesPerCol; j++) {
                ...
                Node* node
                    = new Node(Vec3((double)j / nodesDensity, -((double)i / nodesDensity), 0));
    ...
    void EIcomputeForce(double timeStep, Vec3 gravity) {
        /** Nodes **/
        for (int i = 0; i < nodes.size(); i++) {
    ...
    void EIintegrate(double airFriction, double timeStep) {
        /** Node **/
        for (int i = 0; i < nodes.size(); i++) {
